...
castle_descent = function(){
  ...
  #Pretty cool solution from Alex Rossell Hayes on stackoverflow to yuse RStudio's api to dynamically recieve the console inputs
  ...
  read_console_try_again = function(){
    ...
    while (rstudioapi::isAvailable()) {
      ...
      if(player_retry %in% moves[6]){
        rstudioapi::sendToConsole("", execute = F)
        castle_descent()
      }
      else if (player_retry %in% moves[7]){
  ...
  read_console_movements = function() {
    ...
    read_console_monster_options = function() {
      ...
      while (rstudioapi::isAvailable()) {
        ...
        if (player_monster_choice %in% moves[4:5]) {
          rstudioapi::sendToConsole("", execute = F)
          return(noquote(""))
        }
