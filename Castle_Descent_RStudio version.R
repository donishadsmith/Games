...
#Can input wasd movement controls without pressing enter
...
castle_descent = function(){
  ...
  #Pretty cool solution from Alex Rossell Hayes on stackoverflow to get RStudio to dynamically read the console
  #https://stackoverflow.com/questions/56989889/is-it-possible-to-use-the-r-function-readline-without-requiring-the-user-to-pres
  read_console_try_again = function(){
    ...
    while (rstudioapi::isAvailable()) {
      ...
      if(player_retry %in% moves[8:9]){
        rstudioapi::sendToConsole("", execute = F)
        castle_descent()
      }
      else if (player_retry %in% moves[10:11]){
  ...
  read_console_movements = function() {
    print('w (up), a (left), s (down), or, d (right). You can quit by pressing ctrl + c.', quote = F)
    while (rstudioapi::isAvailable()) {
      player_input <<- tolower(rstudioapi::getConsoleEditorContext()$contents)
      
      if (player_input %in% moves[1:4]) {
        rstudioapi::sendToConsole("", execute = F)
        return(noquote(""))
      }
      rstudioapi::sendToConsole("", execute = F)
    ...
    read_console_monster_options = function() {
      print('attack(a) or run(r). You can quit by pressing ctrl + c', quote = F)
      while (rstudioapi::isAvailable()) {
        player_monster_choice <<- tolower(rstudioapi::getConsoleEditorContext()$contents)
        
        if (player_monster_choice %in% moves[4:7]) {
          rstudioapi::sendToConsole("", execute = F)
          return(noquote(""))
        }
  ...
  #Proportion of the empty spaces that will be monsters, genies, or fairies
  monster_proportion = round(length(which(castle == '\u2800'))*0.30,0)
  fairy_proportion = round(length(which(castle == '\u2800'))*0.05,0)
  genie_proportion = round(length(which(castle == '\u2800'))*0.05,0)
